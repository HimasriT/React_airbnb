...
function Slide({ property, linkClicked }) {
    ...
    property.forEach((video) => {
        if (video._id === linkClicked) {
            rows.push(video);
        }
    });
    return (
        rows.map((p) => (
            <div class="card" id={p._id} style={{ marginBottom: 5 + 'em', minHeight: 24 + 'em' }} >
                <div id="carouselExampleSlides" class="carousel slide" data-ride="carousel" >
                    <div class="carousel-inner">
                        <div class="carousel-item active">
                            <img class="card-img-top d-block w-100" id={p._id} src={p.path} alt="First slide" />
                        </div>
                ...
                <div class="card-body">
                    <h5 class="card-title"><span class="start">{p.name} - {p.city_name}</span><span class="end">{p.ratings}</span><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-star-half" viewBox="0 0 16 16">
            ...
            <ul class="navbar-nav me-auto mb-2 mb-lg-0 list-group-horizontal ">
                <div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
                    <div class="input-group">
                        <input type="text" value={filterText} class="form-control" placeholder="Search by City/Property..."
...
function Properties({ property, filterText, linkClicked, onlinkClicked }) {
    ...
    //  buttonClicked(false);
    property.forEach((video) => {
        var searchText = video.name + " - " + video.city_name;
        if (
            searchText.toLowerCase().indexOf(
                    ...
                    <div class="col-3">
                        <div class="card" id={p._id} style={{ marginBottom: 5 + 'em', minHeight: 24 + 'em' }} >
                            <div id="carouselExampleSlidesOnly" class="carousel slide" data-ride="carousel" >
                                <div class="carousel-inner">
                                    <div class="carousel-item active">
                                        <img class="card-img-top d-block w-100" id={p._id} src={p.path} alt="First slide" onClick={(e) => onlinkClicked(e.target.id)} />
                                    </div>
                                    <div class="carousel-item">
                            ...
                            <div class="card-body">
                                <h6 class="card-title">{p.name} - {p.city_name}</h6>
                                <p class="card-text">
                                    <small class="text-muted">
                                        <b>Ratings: &nbsp;&nbsp;&nbsp; {p.ratings}<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-star-half" viewBox="0 0 16 16">
                                            <path d="M5.354 5.119 7.538.792A.516.516 0 0 1 8 .5c.183 0 .366.097.465.292l2.184 4.327 4.898.696A.537.537 0 0 1 16 6.32a.548.548 0 0 1-.17.445l-3.523 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256a.52.52 0 0 1-.146.05c-.342.06-.668-.254-.6-.642l.83-4.73L.173 6.765a.55.55 0 0 1-.172-.403.58.58 0 0 1 .085-.302.513.513 0 0 1 .37-.245l4.898-.696zM8 12.027a.5.5 0 0 1 .232.056l3.686 1.894-.694-3.957a.565.565 0 0 1 .162-.505l2.907-2.77-4.052-.576a.525.525 0 0 1-.393-.288L8.001 2.223 8 2.226v9.8z" />
                                        </svg></b><br />
                                        {p.host}<br>
                                        </br>
